...
```text
โจทย์ต้องการให้เช็คว่าเวลาที่อยากรู้มีร้านไหนที่เปิดบ้าง
Input : จำนวนร้าน เวลาที่ต้องการเช็ค
เวลาเปิด ปิดของแต่ละร้าน
เวลาที่ต้องการเช็คตามจำนวน
Output : เวลาที่เช็คมีร้านเปิดกี่ร้าน
Constaints : Input จำนวนกับเวลาที่เช็ค อยู่บรรทัดเดียวกัน เวลาเปิดปิด 1 บรรทัดต่อร้าน เวลาเช็คบรรทัดเดียวกัน และ Output บรรทัดเดียวกันหมด
```
...
```text
Step1 : รับค่าจำนวนร้านกับจำนวนเวลาที่ต้องการเข้ามา
Step2 : รับค่าเวลามาแล้วใส่ split เพื่อให้แยกเป็น list และใส่ใน list ชื่อ time อีกที
Step3 : รับค่าเวลาเข้ามาใน list ชื่อ time_C 
Step4 : loop ตัว time_C เพื่อดึงตัวเวลาที่ต้องการมา และ loop time เพื่อดึงเวลาแต่ละร้านมาดู
Step5 : เช็คว่าเวลาที่ต้องการดู อยู่ใน range ของร้านนั้นหรือไม่ ถ้าอยู่ให้ count_n += 1 และเมื่อเช็คหมดแล้วให้เพิ่มเข้าใน list
Step6 : Output  list ชื่อ count ที่ unpack แล้ว
```
...
```text
เหมือนแผนแรกคือรับค่าเข้ามา ใส่ใน  list แล้วก็ loop เพื่อเช็๕ว่าแต่ละค่า และแสดง output
```
...
```text
ทดสอบการนับพื้นฐานที่เวลาอยู่ในหลายช่วงและช่วงเดียว
```
...
```text
ทดสอบว่า range ไม่รวม end value
```
...
```text
ทดสอบในเคสที่ไม่อยู่ในค่าไหนเลย
```
