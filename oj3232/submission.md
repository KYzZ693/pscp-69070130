...
```text
input : ระยะที่กระโดดได้ x และระยะทางที่ต้องไปให้ถึง y
output : จำนวนครั้งที่กระโดด หรือ -1 ถ้าไปไม่ถึง
constraints : กระโดดแล้วระยะทาง -2 ทุกครั้ง
```
...
```text
step 1 : รับค่าเข้ามา แล้ว cast เป็น int 
step 2 : เข้า while loop โดยมี condition ว่า จะวน loop ถ้า ระยะที่กระโดดได้น้อยกว่าระยะที่ต้องการ
step 3 : เช็คว่าระยะกระโดดเหลือน้อยกว่า 0 หรือยัง ถ้าใช่ก็แทนจำนวนครั้งที่กระโดดด้วย -1 และ break ออก
step 4 : ให้ทำการเพิ่มระยะที่กระโดดได้ตามจำนวนที่มี เพิ่มจำนวนครั้งที่กระโดด และ ลดระยะกระโดด
step 5 : output จำนวนครั้งออกมา
```
...
```text
เหมือนกับแผนแรกคือระบค่าเข้ามาละเข้า while loop และเช็คค่า ทำการคำนวณ และเมื่อ condition ของ loop เป็น false ให้ output จำนวนครั้งที่กระโดด หรือ -1 ออกมา
```
...
```text
เช็คว่าถ้าระยะกระโดดเกินจะออกหรือไม่
```
...
```text
เช็คว่าถ้าระยะไม่ถึงแต่ต้นจะออกหรือไม่
```
