...
```text
โจทย์ให้ตรวจสอบว่าจุด(บ้าน)จะโดนหมึกจมที่กี่วินาที
Input : จำนวนพื้นที่ที่ขยายต่อวินาที กับ จำนวนคนที่อยากรู้
Output : วินาทีที่บ้านโดนหมึกจม
Constraints : เมื่อ output ออกมามีหน่วยวินาทีที่ไม่ใช่จำนวนเต็มให้ปัดขึ้นทุกกรณี
```
...
```text
step1 : รับค่าข้อมูลของการขยายพื้นที่น้ำหมึกและจำนวนคนที่ต้องการรู้เวลาที่บ้านตัวเองโดนจม
step2 : หาอัตราส่วนของรัศมีที่เพิ่มขึ้นต่อวินาทีด้วยการหาร pi
step3 : วน loop รับค่าพิกัดบ้าน
step4 : คำนวณหารัศมีที่จะหมึกถึงบ้าน 
step5 : output ตัววินาทีที่ถูกต้องและปัดขึ้นด้วย ceil
```
...
```text
เหมือนแผนแรกคือการหาอัตรารัศมีต่อวินาทีละเอามาหาวินาทีที่รัศมีจะถึงที่ระบุของบ้านลูกค้า
```
...
```text
เพื่อเทสว่าโปรแกรมสามารถรันได้ในตำแหน่งพิกัด(0,0)
```
...
```text
เพพื่อเช็คว่าระบบคำนวณใช้งานได้ปกติ
```
...
```text
เพื่อเช็คว่าสามารถรันได้หลาย input และถูกต้อง
```
